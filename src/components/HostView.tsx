...
export function HostView({ roomCode, onBack }: HostViewProps) {
  ...
  // Only treat an active capture as "sharing". A failed attempt must fall back
  // to the start screen, otherwise the user is left staring at a dead preview
  // with no way to see the error or retry.
  const isSharing = status !== 'idle' && status !== 'failed';
          ...
          <div className="flex-1 flex flex-col gap-6">
            ...
            <div className="flex-1 bg-slate-900 border border-slate-800 rounded-2xl overflow-hidden relative min-h-[300px]">
                ...
                <div className="absolute inset-0 flex items-center justify-center bg-slate-950/70">
                  <div className="text-center">
                    ...
                    <p className="text-sm text-slate-400">
                      Starting capture — allow screen sharing in the browser
                      prompt
                    </p>
                  </div>
                </div>
              ...
              {status === 'connected' && viewerCount === 0 && (
                <div className="absolute bottom-4 left-1/2 -translate-x-1/2 px-4 py-2 rounded-full bg-slate-950/70 backdrop-blur-sm border-slate-800">
                  <p className="text-xs text-slate-300">
                    You are live — waiting for viewers to join with code{
                      ' '
                    }
